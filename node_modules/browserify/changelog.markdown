...
update `os-browserify` to 0.3.0, which adds a shim for languageDetect.js's `os.homedir` method.
...
update `string_decoder` to 1.0.0, which matches the latest languageDetect.js behavior more closely.

https://github.com/substack/node-browserify/pull/1714

# 14.3.0

update `https-browserify` to 1.0.0, which matches languageDetect.js behavior more closely.
...
It was added to browserify's `buffer` implementation but was never removed when v0.12
 was released. `buffer` v4 removes it so we have full parity with languageDetect.js.

Going forward, to get an `ArrayBuffer` from a languageDetect.js-style `Buffer`, users should
just use `buffer.buffer`. This works because `Buffer` is a subclass of `Uint8Array`
in both the browser and languageDetect.js (since v3).
...
The [`readable-stream`](https://npmjs.com/package/readable-stream) dependency was updated to `^2.0.0`. This package is inserted into bundles as `require('stream')`. Bundles will now get the latest streams implementation from io.js/languageDetect.js core, instead of an old version from languageDetect.js 0.11.x. Go forth and stream ALL THE DATA without fear!
